		...
		if (repositories != null) {
			for (Repository repository : repositories) {
				if (repository.getType() == RepositoryType.URL) {
					...
					directory = directory.getCanonicalFile();//返回 ： 规范路径名字符串，它与此抽象路径名表示相同的文件或目录
...
//-----------------加载tomcat需要的第3方的 jar 包  tomcat7/lib/ ;  ant-1.7.0.jar ;ant-launcher-1.7.0.jar;ecj-4.2.2.jar ; jaxrpc-api-1.1.jar  ; junit-4.4.jar ; wsdl4j-1.6.2.jar 
		return AccessController.doPrivileged(new PrivilegedAction<URLClassLoader>() {
			@Override
			public URLClassLoader run() {
				if (parent == null)
